...
/-!
Static ERC-2981 consumer. The constructor immutable is the royalty receiver. A zero receiver is
an invalid royalty configuration, so that deployment advertises IERC165 only. The fee is a
...
Precondition for an advertised royalty profile: the immutable receiver is nonzero. -/
@[pf_entry]
...
/-- Explicit bounded ERC-165 declaration. A valid nonzero receiver enables IERC2981; a zero
receiver fails closed to IERC165 only. -/
@[pf_entry]
def supportsInterface (_s : State) (interfaceId : Bytes4) : Bool :=
  if Address.isZero Immutable.address then
