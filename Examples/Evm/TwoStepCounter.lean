...
The success path emits `OwnershipTransferred(address(0), owner)`. `transferOwnership(0)`
nominates the zero address (OZ cancel).
-/
...
/-- Step 1 of ownership transfer: current owner nominates `candidate`.
    Non-owner → `OwnableUnauthorizedAccount(caller)`. Zero candidate nominates zero (OZ cancel).
