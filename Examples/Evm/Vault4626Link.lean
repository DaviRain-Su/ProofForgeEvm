...
/-!
Bounded ERC-4626 vault consumer: compile-time fixed underlying asset, 1:1 share ledger,
closed ERC-20 asset movement, and ordered reentrancy lock around external calls.
-/
...
@[pf_entry]
def convertToShares (_s : State) (assets : UInt256) : UInt256 :=
  Erc4626.convertToShares Immutable.address assets

@[pf_entry]
def convertToAssets (_s : State) (sharesAmt : UInt256) : UInt256 :=
  Erc4626.convertToAssets Immutable.address sharesAmt

@[pf_entry]
def previewDeposit (_s : State) (assets : UInt256) : UInt256 :=
  Erc4626.previewDeposit Immutable.address assets

@[pf_entry]
def previewRedeem (_s : State) (sharesAmt : UInt256) : UInt256 :=
  Erc4626.previewRedeem Immutable.address sharesAmt
  ...
  if Erc4626.canVault Immutable.address then
    if Reentrancy.canEnter s.guard then
      if !Address.isZero receiver &&
          Fungible.Balances.canCredit shares receiver assets then
        ...
        .ok ({ totalShares := UInt256.add s.totalShares assets,
               guard := Reentrancy.notEntered,
               dummy := Fungible.Balances.credit shares receiver assets },
          Erc4626.Log.deposit Context.caller receiver assets assets)
    ...
    if Reentrancy.canEnter s.guard then
      if Address.eq Context.caller owner then
        if !Address.isZero receiver &&
          ...
          let assets := Erc4626.convertToAssets Immutable.address sharesAmt
          let _ := Reentrancy.enter declared.handle.guard
