...
- 32 backlog coverage rows: 2 DONE, 21 PARTIAL, 9 ABSENT (all 9 blocked by non-goals; no
  ...
  `assets * (totalSupply + 1) / (totalAssets + 1)` via full-precision `mulDiv` with virtual +1,
  ceiling `previewMint` via `mulDivCeil`, ceiling `previewWithdraw` via `mulDivCeil`, and fixed-id ERC-6909.
