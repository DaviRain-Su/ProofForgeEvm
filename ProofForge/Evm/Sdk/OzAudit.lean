...
- 32 backlog coverage rows: 2 DONE, 21 PARTIAL, 9 ABSENT (all 9 blocked by non-goals; no
  ...
  Row 2 (`access/extensions`, `access/manager`) stays PARTIAL after `AdminDelayLink` shipped
  ...
  `assets * (totalSupply + 10) / (totalAssets + 1)` via full-precision `mulDiv` with compile-time
  `_decimalsOffset() == 1` (virtual shares 10), ceiling `previewMint` via `mulDivCeilOffsetRev`,
  ceiling `previewWithdraw` via `mulDivCeilOffset`, and fixed-id ERC-6909.
