  ...
  | none =>
    ...
    | none =>
      ...
      if (isConstNamed addends ``HAdd.hAdd || isConstNamed addends ``Nat.add ||
        ...
        | _, _ => none
      else if (isConstNamed addends ``HSub.hSub || isConstNamed addends ``Nat.sub ||
          endsWith addends ".hSub") && addends.getAppArgs.size ≥ 2 then
        let args := addends.getAppArgs
        match foldStaticNat? env fuel args[args.size - 2]! with
        | some n =>
          if n ≥ UInt64.size then
            match ofNatNatArg? env fuel args[args.size - 1]! with
        ...
        | none =>
          ...
          | some m =>
            if m ≥ UInt64.size then
              ...
              | some _ => some (.lit 0)
              | none =>
                match ofNatNatArg? env fuel args[args.size - 2]! with
  ...
  if (isConstNamed e ``Eq || isConstNamed e ``BEq.beq || isConstNamed e ``Ne ||
    ...
    if args.isEmpty then none
      ...
      let ternaryQuery? : Option Evm.WideWord.Query :=
        ...
        else if isConstNamed baseE ``ProofForge.Evm.Runtime.evmMulDivCeilOffset256 ||
    ...
    if proj == "mk" || proj == "ok" || proj == "error" ||
      ...
      if skipVector then none
        ...
        | none =>
          ...
          | _ => none
  else if isConstNamed e ``UInt32.ofNat && e.getAppArgs.size ≥ 1 then
    match asVal env fuel e.getAppArgs[e.getAppArgs.size - 1]! with
...
private def opOfRuntimeApp (env : Environment) (app : Expr) : Option Ops.Op :=
  ...
  else if isConstNamed app ``ProofForge.Evm.Runtime.evmRevertOwnableInvalidOwner ||
      endsWith app ".evmRevertOwnableInvalidOwner" then
    match nthFromEnd args 0 with
    | some owner =>
        ...
        if let some condE := findBy args (fun a =>
          ...
          if let .ok thn := decodedThen then
            if thn.any fun | .letLocal .. => true | _ => false then
              ...
              | none => pure ()
          match asCmp env condE, decodeEvmEffect env t, asIndexSet env t,
              asStoreFields env t true, asOkState env t, decodedThen with
          | some (cmp, lv, rv), some evmOps, _, _, _, _ =>
          ...
          match asCheckedAddGuard env condE, decodeEvmEffect env t,
          ...
          match asCheckedSubGuard env condE, decodeEvmEffect env t,
              decodeExpr env fuel' t (stateful := stateful)
                (preserveLocals := preserveLocals) (localDepth := localDepth)
                (stateType? := stateType?) (deepScalars := deepScalars),
