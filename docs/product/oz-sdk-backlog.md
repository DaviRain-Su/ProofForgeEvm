...
| W4 | Static metadata/finance/crypto profiles: bounded ERC-721/1155 URI response, EIP-5267-style static domain fields, single-beneficiary vesting, bounded Merkle proofs | Slice 1 verified on `cursor/oz-sdk-w4-e4eb`: `Sdk.MetadataUri` (capacity 32, `canRespond721`/`canRespond1155`, `select721`/`select1155`, `empty`), `ArtLink` static `tokenURI` + `anvil_artlink.sh`, `Tests.EvmMetadataUriSpec`. Extract accepts encodable-id length gates; mint-state gate wiring is slice 1b. |
