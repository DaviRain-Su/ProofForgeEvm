...
| 1 | Richer typed call shapes for composition: STATICCALL returns typed as `Bool` and `Address`, three and four exact words, and one bounded `bytes` argument. | Shipped: `OpenCall.StaticShape` (`word`, `words2`, `words3`, `words4`, `bool`, `address`) names each read's `CallResult.Policy` and limb count; `Sdk.OpenCall.staticWords3` / `staticWords4` / `staticBool` / `staticAddress`; `Decode` recognizes the stubs through one name table; `CallResult.Emit.wordLimb` projects an address word into `Addr20` byte limbs (shared with `ecrecover20`). Reads stay entry-return only, as `staticWord` already was. Open: the bounded `bytes` argument. It needs an `Arg` kind with a calldata tail (`offset`, `length`, padded data), a runtime-length or capacity-padded `inSize`, and the `bytes` ABI type in the selector. Phase 3's `isValidSignature(bytes32,bytes)` is its first consumer, so it lands with or just before Phase 3. | `Tests.EvmOpenCallSpec` pins policy and limb count per shape, the bool/address word gates, the 96- and 128-byte frame gates, and the limb count each entry binds; `anvil_opencall.sh` proves each fail-closed frame (bool word `2`, dirty address high bytes, wrong-size frames, EOA); `anvil_compose.sh` reads `balanceOf`, `ownerOf`, and a `pf`-compiled `Badge`'s `supportsInterface` (`true` and `false`) between `pf`-compiled contracts | None. Arbitrary calldata stays out; the pending tail is one typed bounded argument |
