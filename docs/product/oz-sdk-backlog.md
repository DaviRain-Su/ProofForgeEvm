...
| W3 | Bounded access/utility closeout: Ownable2Step start event and constructor-init policy where extractable; fixed-capacity role profiles; bounded nonce/rate helpers | In progress on `cursor/oz-sdk-w3-e4eb`: slice 1 merged (PR #18, Ownable2Step); slice 2a ships `Sdk.Nonces`; `RateLimit`/`EvmQuota` and `Roles.Set4`/`EvmCrew` remain |
