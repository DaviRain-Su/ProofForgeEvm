...
| `token/ERC20/**` | PARTIAL | `Sdk.Fungible`, `Sdk.Erc20Meta`, `Sdk.SafeErc20`, `Erc20Meta`, `Token`, `SafePay` | No complete extensions/permit-votes ecosystem; `Token` keeps packed `bytes32` metadata | Yes — W5 slice 4 shipped `Sdk.Erc20Meta` + `Fungible.Log` typed Transfer/Approval on `Erc20Meta`; extensions/permit-votes remain blocked |
