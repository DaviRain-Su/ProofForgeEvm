...
| `utils/Blockhash.sol`, `BlockHeader.sol`, `ERC6372Utils.sol`, `Nonces*.sol`, `RateLimiter.sol` | PARTIAL | `Sdk.Nonces`, `Context.blockHash`, block fields, hashed/static storage | Fixed-window rate limit and header utility profiles not shipped | Rate limit profile: next W3 slice; header profiles: W4 or blocked |
...
| W3 | Bounded access/utility closeout: Ownable2Step start event and constructor-init policy where extractable; fixed-capacity role profiles; bounded nonce/rate helpers | In progress on `cursor/oz-sdk-w3-e4eb`: slice 1 merged (PR #18, Ownable2Step); slice 2a ships `Sdk.Nonces`; slice 2b ships `Sdk.RateLimit`; `EvmQuota` and `Roles.Set4`/`EvmCrew` remain |
