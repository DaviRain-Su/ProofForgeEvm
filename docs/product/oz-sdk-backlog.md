...
| `finance/VestingWallet*.sol` | PARTIAL | `Sdk.Vesting`, `VestLink`, `Context.timestamp` | ERC-20 vesting map, beneficiary rotation, parameterless release mutation | Yes — W4 slice 3 shipped bounded native-ETH single-beneficiary profile |
...
| `interfaces/IERC5267.sol`, `IERC5313.sol`, `IERC6372.sol`, `IERC5805.sol` | PARTIAL | `Context`, `Permit`, `Sdk.Eip712Domain`, `DomainLink` | EIP-712 domain, owner, clock/votes views | Static domain fields: W4 slice 2 shipped; owner/clock/votes blocked by votes model |
