...
Named S4 restriction: `RoleAdminChanged` (no role-admin rotation API). Constructor `OwnershipTransferred` shipped on TwoStepCounter, Credits, VestLink, and Vest20Link. ERC-1155 `TransferBatch` shipped bounded in Phase 2 (see the OZ SDK backlog). S4a–d (721/Ownable-Pausable/1155/Roles events) landed on `main` as #11–#14. W3 verified on `cursor/oz-sdk-w3-e4eb`: Ownable2Step start event + constructor-init policy; slice 2 (`Nonces`/`RateLimit`/`EvmQuota`, `Set4`/`EvmCrew`).
