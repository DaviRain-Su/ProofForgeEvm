...
| `Ownable.Log` + `TwoStepCounter` / `Credits` | Canonical `OwnershipTransferred` (LOG3) on **`acceptOwnership`** and **`renounceOwnership`**, and Ownable2Step `OwnershipTransferStarted` (LOG3) on **`transferOwnership`**. Renunciation clears owner plus the sole `pendingOwner()` nominee. Constructor init stores the owner argument and empty pending state and does not log. VestLink/Vest20 CREATE emits `OwnershipTransferred(address(0), owner)` and reverts `OwnableInvalidOwner(address)` as a dropped-let on those consumers, not on TwoStepCounter/Credits | Drop-in OpenZeppelin Ownable / Ownable2Step |
