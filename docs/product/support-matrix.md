...
| Typed OpenCall (`OpenCall.call` / `callSuccess` / `callValue` / `callMagic` / `staticTryMagic` / `staticWord` / `staticWords2` / `staticWords3` / `staticWords4` / `staticBool` / `staticAddress`) | **Supported** (S3, on `main` as #10; read shapes widened in the Phase 1 expansion PR): CALL/STATICCALL to a typed `Address` target with a compile-time constructor ABI (≤8 args, one-word scalars plus at most two `BoundedVec` array fields and at most one `BoundedBytes n` field with `n ≤ 65`, the packed `bytes` ceiling `Codec.maxPackedBytesCapacity` (one ECDSA signature), sent as ABI `bytes` at its runtime length so the calldata equals `abi.encodeWithSelector`; a plan with an array walks a byte cursor so later tails follow a runtime-length prefix; one packed `BoundedBytes` or `BoundedString` field (`n ≤ 65`) is sent as ABI `bytes` or `string`; a second packed field is refused). Result gates are S2 `CallResult` policies; `callMagic` is the receiver-hook shape, a CALL whose one returned word must be the plan's own selector (`onERC721Received`, `onERC1155Received`, `onERC1155BatchReceived`, ERC-1271 `isValidSignature`), with the magic computed from the constructor; `staticTryMagic` is that same selector as a Bool (`true` on exact magic, `false` otherwise, no call-result revert); STATICCALL reads come back typed as `UInt256`, `Bool`, or `Address` and stand anywhere such a value can: an entry's result, an `if` condition, a comparison operand, or another call's argument. A read used in several limbs runs once. CALL results are effect carriers, not values: a carrier stands only as the entry's result word, alone or under `Effect.thenTrue`; a comparison, an operator, an `if` condition, a state field, a call argument, or a dropped `let` around one is refused at extraction with the `CALL carrier out of place` reason (`Tests/EvmOpenCallMisuse.lean` is the refused fixture; `anvil_opencall.sh` fails if `pf build` compiles it). No raw calldata, selector string, return-buffer length, opcode, `delegatecall`, CREATE2, or proxy. Reentrancy is application-visible. Calling another contract needs no proxy: `runtime-tests/evm/anvil_compose.sh` moves tokens from one `pf`-compiled contract into another through `OpenCall.call`. See [writing-contracts.md](writing-contracts.md#call-another-contract). |
| CallResult S2 (`exactWords n`, `strictBool`, `magicBytes4`, `tryMagicBytes4`, typed `words`) | **Supported** (S2, on `main` as #9): ≤4 ABI words. ClosedCall still uses the original three policies. OpenCall consumes `canonicalTrueOrCodeBackedEmpty` / `contractSuccess` / `exactWord` / `exactWords 2..4` / `strictBool` / `words #[.address20]` / `tryMagicBytes4` via the helpers above. Default fail mode is `revert(0, 0)` |
...
|---|---|---|
| `Erc20Meta` + `Examples.Evm.Erc20Meta` | ERC-20-shaped ABI: `string` name/symbol, standard `allowance` / `transfer` / `approve` selectors, issuer EIP-2612 `permit` / `DOMAIN_SEPARATOR` / `nonces` over the closed Token/1 domain | “Audited EIP-20” / mainnet token factory |
| `Erc3009` + `Auth3009Link` | Bounded ERC-3009 `transferWithAuthorization`, `receiveWithAuthorization`, and `cancelAuthorization` over the closed Token/1 domain. Receive uses a distinct EIP-712 typehash and requires `caller == to`. Cancel uses `CancelAuthorization(address authorizer,bytes32 nonce)` and marks the auth-used slot. `authorizationState(address,bytes32)` is a Bool view of that slot. Driven in `anvil_auth3009link.sh` | Drop-in OpenZeppelin ERC-3009 (remaining draft interfaces) |
...
| `Vesting` + `VestLink` / `Vest20Link` | `VestLink.release()` pays `releasable()` native ETH to the stored beneficiary. `release(uint256)` still allows a partial payout at or below that amount. `released()` is the paid native counter. `transferOwnership` nominates a pending owner (`OwnershipTransferStarted`). `acceptOwnership` rotates that beneficiary (`OwnershipTransferred`). `renounceOwnership` clears owner and pending. `pendingOwner()` is the nominee. `cliffDuration` is constructor-stored. `cliff()` is `start + cliffDuration`. Before the cliff, `vestedAmount` is 0. After the cliff the linear formula still uses `timestamp - start`. CREATE of `address(0)` reverts `OwnableInvalidOwner(address)`. CREATE of a nonzero owner logs `OwnershipTransferred(address(0), owner)`. `transferOwnership(address(0))` nominates the zero address (OZ cancel). Non-owner `transferOwnership` reverts `OwnableUnauthorizedAccount(caller)`. `Vest20Link` is the dual-asset wallet: `receive` plus `release()` for native ETH, and `release(address)` for ERC-20 through `SafeErc20.transfer`, logging `EtherReleased` and `ERC20Released`. `released()` / `released(address)` are the OZ view names. Driven in `anvil_vestlink.sh` and `anvil_vest20link.sh` | Named restricted profile. VestLink is ETH-only. Vest20Link is dual-asset. Not an OZ drop-in |
...
| `Ierc1271` + `SignerLink` | `Ierc1271.checkNow signer hash signature` is OZ `isValidSignatureNow` as a fail-closed carrier. A signer with code takes `checkSignature` (`isValidSignature` via `OpenCall.callMagic`, magic `0x1626ba7e`). A signer without code recovers a 65-byte `r ‖ s ‖ v` through `Ecdsa.recover` and reverts `Unauthorized(signer)` on mismatch. `Ierc1271.validNow` is the same gate as a Bool (`OpenCall.staticTryMagic` on a contract signer; EOA mismatch and a short frame answer `false`). `SignerLink.requireNow` counts accepted fail-closed checks. `tryNow` counts only a `true` Bool. `requireSigner` stays the 1271-only path. Driven in `anvil_signerlink.sh` against an EOA and a Solidity ERC-1271 wallet, including an `extcodesize` mutation | Drop-in OpenZeppelin SignatureChecker (signatures wider than 65 bytes; invalid ECDSA still empty-reverts inside the precompile); an ERC-1271 wallet (the receiving side is a non-goal) |
...
| `Ownable.Log` + `TwoStepCounter` / `Credits` | Canonical `OwnershipTransferred` (LOG3) on **CREATE** (`previousOwner` is zero), **`acceptOwnership`**, and **`renounceOwnership`**, and Ownable2Step `OwnershipTransferStarted` (LOG3) on **`transferOwnership`**. CREATE of `address(0)` reverts `OwnableInvalidOwner(address)`. `transferOwnership(0)` nominates the zero address (OZ cancel). Renunciation clears owner plus the sole `pendingOwner()` nominee. VestLink/Vest20 CREATE uses the same dropped-let and the same `renounceOwnership`. | Drop-in OpenZeppelin Ownable / Ownable2Step |
