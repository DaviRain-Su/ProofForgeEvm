...
| `Vesting` + `VestLink` / `Vest20Link` | `VestLink.release()` pays `releasable()` native ETH to the stored beneficiary. `release(uint256)` still allows a partial payout at or below that amount. `transferOwnership` is one-step Ownable rotation of that beneficiary (`OwnershipTransferred`). `Vest20Link.release(address)` pays `releasable(token)` through `SafeErc20.transfer` to the current beneficiary, logs `ERC20Released`, and stores paid amounts in a hashed token map. Driven in `anvil_vestlink.sh` and `anvil_vest20link.sh` | Drop-in OpenZeppelin VestingWalletCliff / constructor zero-owner revert |
