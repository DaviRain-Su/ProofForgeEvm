...
| Typed OpenCall (`OpenCall.call` / `callSuccess` / `callValue` / `staticWord` / `staticWords2` / `staticWords3` / `staticWords4` / `staticBool` / `staticAddress`) | **Supported** (S3, on `main` as #10; read shapes widened in the Phase 1 expansion PR): CALL/STATICCALL to a typed `Address` target with a compile-time constructor ABI (≤8 args, one-word scalars plus at most one `BoundedBytes n` field with `n ≤ 63`, sent as ABI `bytes` at its runtime length so the calldata equals `abi.encodeWithSelector`; `BoundedString` and a second `bytes` field are refused). Result gates are S2 `CallResult` policies; STATICCALL reads come back typed as `UInt256`, `Bool`, or `Address` and stand anywhere such a value can: an entry's result, an `if` condition, a comparison operand, or another call's argument. A read used in several limbs runs once. CALL results are effect carriers, not values. A comparison around one is not compiled, and a fail-closed refusal of that shape is the next open-call unit. No raw calldata, selector string, return-buffer length, opcode, `delegatecall`, CREATE2, or proxy. Reentrancy is application-visible. Calling another contract needs no proxy: `runtime-tests/evm/anvil_compose.sh` moves tokens from one `pf`-compiled contract into another through `OpenCall.call`. See [writing-contracts.md](writing-contracts.md#call-another-contract). |
...
| `Erc1155` + `MultiToken` / `CraftToken` | Bounded single-id ownership/balance **core**, plus a bounded `balanceOfBatch` on `MultiToken` (at most four pairs; unequal lengths answer `[]` because a view cannot revert) and a bounded `batchTransferFrom` on `MultiToken` (at most four slots, pairwise distinct ids; OZ-shaped `ERC1155InvalidArrayLength` and `ERC1155InsufficientBalance` reverts; OZ log rule, one `TransferBatch` over the submitted slots or `TransferSingle` for a one-slot batch; no receiver hook, hence no `safe` prefix). These two examples emit canonical ERC-1155 `TransferSingle` (LOG4, `id`+`value` data words) and `ApprovalForAll` (LOG3, bool data) via `Erc1155.Log`; `MultiToken` also emits `TransferBatch` (LOG4, two bounded `uint256[]` tails); neither advertises the incomplete IERC1155 interface ID | Full ERC-1155 (safe callbacks, metadata URI, `safeBatchTransferFrom` with `bytes data` and the receiver hook, unbounded batches, duplicate ids in one batch, complete function set) |
