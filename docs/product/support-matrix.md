...
| `Ownable.Log` + `TwoStepCounter` / `Credits` | Canonical `OwnershipTransferred` (LOG3) on **`acceptOwnership`** and **`renounceOwnership`**, and Ownable2Step `OwnershipTransferStarted` (LOG3) on **`transferOwnership`**. Renunciation clears owner plus the sole `pendingOwner()` nominee. Constructor init stores the owner argument and empty pending state. Constructor logs are not lowered. VestLink/Vest20 CREATE `ZeroAddress()` is a dropped-let on those consumers, not on TwoStepCounter/Credits | Drop-in OpenZeppelin Ownable / Ownable2Step |
