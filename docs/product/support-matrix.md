...
| `Erc1155` + `MultiToken` / `CraftToken` | Bounded single-id ownership/balance **core**, plus a bounded `balanceOfBatch` on `MultiToken` (at most four pairs; unequal lengths answer `[]` because a view cannot revert) and a bounded `batchTransferFrom` on `MultiToken` (at most four slots, pairwise distinct ids; OZ-shaped `ERC1155InvalidArrayLength` and `ERC1155InsufficientBalance` reverts; one `TransferSingle` per slot in array order; no receiver hook, hence no `safe` prefix). These two examples emit canonical ERC-1155 `TransferSingle` (LOG4, `id`+`value` data words) and `ApprovalForAll` (LOG3, bool data) via `Erc1155.Log`; neither advertises the incomplete IERC1155 interface ID | Full ERC-1155 (safe callbacks, metadata URI, `safeBatchTransferFrom` with `bytes data` and the receiver hook, `TransferBatch`, duplicate ids in one batch, complete function set) |
