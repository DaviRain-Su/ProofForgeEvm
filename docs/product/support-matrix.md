...
| `Erc3009` + `Auth3009Link` | Bounded ERC-3009 `transferWithAuthorization` and `receiveWithAuthorization` over the closed Token/1 domain. Receive uses a distinct EIP-712 typehash and requires `caller == to`. Driven in `anvil_auth3009link.sh` | Drop-in OpenZeppelin ERC-3009 (cancellation, authorization-state views, remaining draft interfaces) |
