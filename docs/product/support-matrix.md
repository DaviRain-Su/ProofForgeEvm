...
| `Erc2981` + `RoyaltyArt` | Static royalty quote: constructor receiver + compile-time basis points; exact full-range `royaltyInfo(uint256,uint256) → (address,uint256)`; token id ignored; zero receiver disables royalties | Per-token royalty map, metadata URI, or an OpenZeppelin ERC2981 clone |
