...
| `Erc3009` + `Auth3009Link` | Bounded ERC-3009 `transferWithAuthorization`, `receiveWithAuthorization`, and `cancelAuthorization` over the closed Token/1 domain. Receive uses a distinct EIP-712 typehash and requires `caller == to`. Cancel uses `CancelAuthorization(address authorizer,bytes32 nonce)` and marks the auth-used slot. Driven in `anvil_auth3009link.sh` | Drop-in OpenZeppelin ERC-3009 (authorization-state views, remaining draft interfaces) |
