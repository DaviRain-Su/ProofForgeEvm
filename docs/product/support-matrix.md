...
| `Erc165` | Static `bytes4` interface IDs and bounded, explicitly declared support predicates. The four partial token examples advertise `IERC165` only and return false for ERC-721/1155, `0xffffffff`, and other undeclared IDs. `RoyaltyArt` advertises IERC165 + complete IERC2981 only with a nonzero receiver | Runtime discovery, receiver handshakes, inferred interface lists, or claiming an incomplete standard interface |
...
| `Erc721` + `Collectible` / `Badge` | Bounded ownership/approval/balance **core**, plus the outbound receiver check: `Collectible.safeTransferFrom(address,address,uint256,bytes)` moves the token, emits `Transfer`, then runs `Erc721.checkOnReceived` (OZ `checkOnERC721Received` over `OpenCall.callMagic`: a recipient with code must answer `onERC721Received` with its own selector or the whole transaction reverts empty; a recipient without code is not called; `data` at most 32 bytes; no three-argument overload). A receiver reading `ownerOf` inside the hook sees itself. These two examples emit the three canonical ERC-721 events via `Erc721.Log` (`Transfer` LOG4, `Approval` LOG4, `ApprovalForAll` LOG3); neither advertises the incomplete IERC721 interface ID | Full ERC-721 (the receiving side of the hook, metadata URI, complete function set) |
| `Ierc1271` + `SignerLink` | The contract-signer half of OZ `SignatureChecker`: `Ierc1271.checkSignature signer hash signature` CALLs `isValidSignature(bytes32,bytes)` through `OpenCall.callMagic` and reverts unless the one returned word is `0x1626ba7e`; the signature is a `BoundedBytes 65` (one ECDSA `r ‖ s ‖ v`); a signer without code reverts; `SignerLink.requireSigner(address,bytes32,bytes)` counts accepted checks. Driven against a Solidity ERC-1271 wallet in `anvil_signerlink.sh` | OZ `isValidSignatureNow` (no ECDSA fallback inside the check, no `false` answer, no wider multi-signature payload); an ERC-1271 wallet (the receiving side is a non-goal) |
| `Erc1155` + `MultiToken` / `CraftToken` | Bounded single-id ownership/balance **core**, plus a bounded `balanceOfBatch` on `MultiToken` (at most four pairs; unequal lengths answer `[]` because a view cannot revert) and a bounded `batchTransferFrom` on `MultiToken` (at most four slots, pairwise distinct ids; OZ-shaped `ERC1155InvalidArrayLength` and `ERC1155InsufficientBalance` reverts; OZ log rule, one `TransferBatch` over the submitted slots or `TransferSingle` for a one-slot batch; no receiver hook, hence no `safe` prefix). `CraftToken.safeTransferFrom(address,address,uint256,uint256,bytes)` is the single transfer with the outbound receiver check (`Erc1155.checkOnReceived`, OZ `checkOnERC1155Received` over `OpenCall.callMagic`: a recipient with code must answer `onERC1155Received` with its own selector or the whole transaction reverts empty; a recipient without code is not called; `data` at most 32 bytes); a receiver reading `balanceOf` inside the hook sees the credited balance. `MultiToken` keeps the hook-less `transferFrom` because the hook entry does not fit it under EIP-170. These two examples emit canonical ERC-1155 `TransferSingle` (LOG4, `id`+`value` data words) and `ApprovalForAll` (LOG3, bool data) via `Erc1155.Log`; `MultiToken` also emits `TransferBatch` (LOG4, two bounded `uint256[]` tails); neither advertises the incomplete IERC1155 interface ID | Full ERC-1155 (the receiving side of the hooks, metadata URI, `safeBatchTransferFrom` with `bytes data` and `onERC1155BatchReceived`, unbounded batches, duplicate ids in one batch, complete function set) |
