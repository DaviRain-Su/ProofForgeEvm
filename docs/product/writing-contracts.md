...
3. **Constructor effects.** Deployment-time map writes, value transfers, and logs other than constructor `OwnershipTransferred(address(0), owner)` stay refused. A dropped-let `Revert.ownableInvalidOwner` before the `returnState` vector lowers as a constructor prefix. VestLink and Vest20Link CREATE of `address(0)` revert `OwnableInvalidOwner(address)`. The else-arm of that dropped-let emits `Ownable.Log.constructorTransferred`. TwoStepCounter and Credits still store a zero owner without that CREATE revert or log.
