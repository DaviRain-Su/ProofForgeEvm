...
3. **Constructor effects.** Deployment-time map writes, logs, and value transfers stay refused. A dropped-let `Revert.zeroAddress` before the `returnState` vector lowers as a constructor prefix. VestLink and Vest20Link CREATE of `address(0)` revert `ZeroAddress()`. Constructor `OwnershipTransferred(address(0), owner)` does not lower. The selector is `ZeroAddress()`, not OZ `OwnableInvalidOwner(address)`.
