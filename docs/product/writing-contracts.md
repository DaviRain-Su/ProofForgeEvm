...
8. **`Examples.Evm.Auth3009Link`** — bounded ERC-3009 `transferWithAuthorization`, `receiveWithAuthorization`, and `cancelAuthorization`. Receive uses the `ReceiveWithAuthorization` typehash and requires `caller == to`. Cancel uses `CancelAuthorization(address authorizer,bytes32 nonce)` and does not move balance.
