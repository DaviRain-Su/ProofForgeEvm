...
one packed `BoundedBytes n` or `BoundedString n` with `n ≤ 65` (the packed ceiling, one ECDSA
signature), sent as ABI `bytes` or `string` at its runtime length. A plan with an array walks a
byte cursor so a later tail can follow a runtime-length prefix. A second packed field is refused.
A STATICCALL
read is a value. It
...
3. **Constructor effects.** Deployment-time map writes, value transfers, and logs other than constructor `OwnershipTransferred(address(0), owner)` stay refused. A dropped-let `Revert.ownableInvalidOwner` before the `returnState` vector lowers as a constructor prefix. VestLink, Vest20Link, TwoStepCounter, and Credits CREATE of `address(0)` revert `OwnableInvalidOwner(address)`. The else-arm of that dropped-let emits `Ownable.Log.constructorTransferred`. VestLink, Vest20Link, TwoStepCounter, and Credits `transferOwnership(0)` nominates the zero address (OZ cancel).
4. **Events** — closed LOG helpers (`Event.tipped` / `Event.transfer`) plus typed constructors via `Event.emit` and `Event.Indexed` (S1b; see `Examples.Evm.EvmTypedEvents`). Product typed events are **named ABI events** (LOG1–4): topic0 is always the signature hash; ABI JSON is `"anonymous": false`. That is not anonymous LOG0 (a plan-layer geometry only, not a source `Event.emit` shape). Closed ERC-20 `Transfer`/`Approval` are LOG3. `Collectible` / `Badge` emit canonical ERC-721 `Transfer` / `Approval` / `ApprovalForAll` through `Erc721.Log`. `MultiToken` / `CraftToken` emit canonical ERC-1155 `TransferSingle` / `ApprovalForAll` through `Erc1155.Log`; `MultiToken.safeBatchTransferFrom` also emits `TransferBatch`, whose two `uint256[]` fields are bounded dynamic-array tails. A typed event may end with at most two non-indexed `BoundedVec` fields over a closed scalar (at most 63 elements each); they lower to the ABI head-offset plus length-prefixed tail layout, and an indexed array or a scalar declared after an array fails extraction. `TwoStepCounter` / `Credits` emit Ownable2Step `OwnershipTransferStarted` on `transferOwnership` and Ownable `OwnershipTransferred` on CREATE, `acceptOwnership`, and `renounceOwnership`; renunciation clears owner and pending nominee. They also emit Pausable `Paused`/`Unpaused`; `Capped` emits the pause pair. CREATE of a zero owner reverts `OwnableInvalidOwner(address)`. `transferOwnership(0)` nominates the zero address (OZ cancel). `EvmStaticCounter` / `EvmStaticRoster` emit canonical AccessControl `RoleGranted` / `RoleRevoked` through `Roles.Log` on actual grant/revoke (no `RoleAdminChanged`); `EvmCrew` extends the same pattern to `Roles.Set4`. `EvmQuota` combines `Nonces` and `RateLimit`; stale nonces revert as `Insufficient(current,provided)` and rate exhaustion uses typed `rateLimitExceeded()` (no quota events). Unbounded event payloads are still refused.
5. **Typed external CALL** — `OpenCall.call` / `callSuccess` / `callValue` / `callMagic` / `staticWord` / `staticWords2` / `staticWords3` / `staticWords4` / `staticBool` / `staticAddress` take a typed `Address` and an inductive constructor (name = ABI function, fields = args; at most two fields may be `BoundedVec` arrays, and one field may be a `BoundedBytes n` or `BoundedString n` with `n ≤ 65`, sent as ABI `bytes` or `string` at its runtime length). The compiler assembles calldata and applies one `CallResult` policy. This is not raw CALL: no selector string, caller-built calldata buffer, `delegatecall`, or CREATE2. Reentrancy is visible to the callee; OpenCall does not insert a guard (see `Examples.Evm.EvmOpenCall`).
6. **NFT modules.** Bounded cores with the three canonical ERC-721 events on Collectible/Badge and the two canonical single-id ERC-1155 events on MultiToken/CraftToken. These advertise IERC165 only; incomplete ERC-721/1155 method surfaces must not claim the standard token ids. `MultiToken` adds a bounded `balanceOfBatch` and `safeBatchTransferFrom` over at most four slots (OZ-shaped `ERC1155InvalidArrayLength` and `ERC1155InsufficientBalance` reverts, `DuplicateId()` when two slots name the same id rather than OZ applying those slots in order, OZ log rule of one `TransferBatch` over the submitted slots or `TransferSingle` for a one-slot batch, then the outbound `onERC1155BatchReceived` check). `Collectible.safeTransferFrom`, `Collectible.safeTransferFrom__id`, `CraftToken.safeTransferFrom`, and `MultiToken.safeTransferFrom` run the outbound receiver check (`Erc721.checkOnReceived` / `Erc1155.checkOnReceived` over `OpenCall.callMagic`): the ledger move and the log land first, a recipient without code is not called, and a recipient with code must answer the hook with its own selector or the whole transaction reverts; `data` is at most 32 bytes. `safeTransferFrom__id` is the three-argument ERC-721 overload (empty `data`). `MultiToken.safeBatchTransferFrom` is the same shape with `Erc1155.checkOnBatchReceived`. `ReceiverLink` is the receiving side. `onERC721Received`, `onERC1155Received`, and `onERC1155BatchReceived` return those selectors as `bytes4`, record packed operator/from/id/value plus `data` length, and advertise IERC165 plus IERC721Receiver and IERC1155Receiver. `anvil_receiverlink.sh` drives Collectible, CraftToken, and MultiToken into it (2,145 runtime bytes). `Gallery` adds bounded IERC721Enumerable (`totalSupply`, `tokenByIndex`, `tokenOfOwnerByIndex`) over UInt64 ids with capacity 4, owner-only transfer and burn, and IERC165 only. Not "ship ERC-721/1155" (no unbounded batches, duplicate ids revert `DuplicateId()` instead of OZ in-order application, unbounded IERC721Enumerable stays out).
7. **`Examples.Evm.Token` metadata** — `name` / `symbol` are packed `bytes32` (Anvil gates use that shape). Prefer `Erc20Meta` when external tools expect ERC-20 `string` metadata. `Erc20Meta` also ships issuer EIP-2612 `permit` / `DOMAIN_SEPARATOR` / `nonces` over the closed Token/1 domain (the string name happens to be `Token`).
8. **`Examples.Evm.Auth3009Link`** — bounded ERC-3009 `transferWithAuthorization`, `receiveWithAuthorization`, and `cancelAuthorization`. Receive uses the `ReceiveWithAuthorization` typehash and requires `caller == to`. Cancel uses `CancelAuthorization(address authorizer,bytes32 nonce)` and does not move balance. `authorizationState(address,bytes32)` is a Bool view of the auth-used slot.
9. **ERC-20 consumers.** `SafeErc20` / `SafePay` wrap the closed token CALLs. They are not a raw-calldata escape hatch and do not catch-and-retry a failed CALL. `forceApprove` always emits `approve(0)` then `approve(amount)`. `Vest20Link.release(address)` pays `releasable(token)` through `SafeErc20.transfer` to the stored beneficiary, logs `ERC20Released`, and stores paid amounts in a hashed token map. `Vest20Link.release()` pays `releasable()` native ETH on the same contract. `VestLink.release()` is the ETH-only smaller profile. Both rotate the beneficiary with Ownable2Step `transferOwnership` plus `acceptOwnership` plus `renounceOwnership`. `transferOwnership(0)` nominates the zero address (OZ cancel). Both take a constructor-stored `cliffDuration`. `cliff()` is `start + cliffDuration`. Before the cliff, vested amount is 0. CREATE of a zero beneficiary reverts `OwnableInvalidOwner(address)`. CREATE of a nonzero beneficiary logs `OwnershipTransferred(address(0), owner)`. Non-owner `transferOwnership` reverts `OwnableUnauthorizedAccount(caller)`. `Vest20Link.release(address)` still reverts `ZeroAddress()` on a zero token.
`released()` and `released(address)` are the OZ view names.
10. **ERC-2981** — `RoyaltyArt` is a static receiver + basis-point quote over the full UInt256 sale-price range. `tokenId` is ignored; a zero receiver is invalid and causes the deployment to advertise IERC165 only. There is no per-token royalty map or metadata URI.
