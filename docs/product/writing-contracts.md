...
3. **Constructor effects.** Deployment-time map writes, value transfers, and logs other than constructor `OwnershipTransferred(address(0), owner)` stay refused. A dropped-let `Revert.zeroAddress` before the `returnState` vector lowers as a constructor prefix. VestLink and Vest20Link CREATE of `address(0)` revert `ZeroAddress()`. The else-arm of that dropped-let emits `Ownable.Log.constructorTransferred`. The selector is `ZeroAddress()`, not OZ `OwnableInvalidOwner(address)`.
