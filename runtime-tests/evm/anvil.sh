...
for case in counter pair flag maybe ctx bounded search find_index static_counter static_roster evmcrew evmquota \
    ...
    ownable token erc20meta safepay royaltyart artlink packlink domainlink vestlink prooflink auditlink headerlink admindelaylink auth3009link vault4626link ninelink window phase wide const capped multitoken crafttoken twostep_counter credits \
    opencall; do
