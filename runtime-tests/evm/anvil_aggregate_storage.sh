...
pf_evm_evm_init evm-anvil-aggregate-storage
...
pf_evm_start_anvil "${PF_EVM_PORT:-18567}" "$root/build/evm/anvil-aggregate-storage.log"
...
for slot in 3 4 5; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero bundle leaf"
done
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'adminOf()(address)')" \
  "$sender" "admin getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'amountOf()(uint64)')" \
  0 "constructor amount view"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'sideOf()(uint8)')" \
  0 "constructor side view"
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'enabledOf()(bool)')" \
...
pf_evm_require_equal "$signal0_values" "0 false" "constructor bundleSignal"
...
pf_evm_require_equal "$view0_values" "0 0 false" "constructor bundleView"
...
pf_evm_require_equal "$details0_values" "0 false" "constructor detailsView"
...
pf_evm_require_storage "$addr" 3 11 "setBundle amount"
pf_evm_require_storage "$addr" 4 4 "setBundle details.side"
pf_evm_require_storage "$addr" 5 1 "setBundle details.enabled"
pf_evm_require_storage "$addr" 0 "$admin_w0" "setBundle holds admin.w0"
pf_evm_require_storage "$addr" 1 "$admin_w1" "setBundle holds admin.w1"
pf_evm_require_storage "$addr" 2 "$admin_w2" "setBundle holds admin.w2"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'amountOf()(uint64)')" \
...
pf_evm_require_equal "$signal1_values" "11 true" "bundleSignal after setBundle"
...
pf_evm_require_equal "$view1_values" "11 4 true" "bundleView after setBundle"
...
pf_evm_require_equal "$details1_values" "4 true" "detailsView after setBundle"
...
pf_evm_require_equal "$pairs1_values" "11 4" "amountSidePairs after setBundle"

# Targeted amount update must preserve nested details sibling leaves.
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setAmount(uint64)' 42 >/dev/null
pf_evm_require_storage "$addr" 3 42 "setAmount amount"
pf_evm_require_storage "$addr" 4 4 "setAmount holds details.side"
pf_evm_require_storage "$addr" 5 1 "setAmount holds details.enabled"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'amountOf()(uint64)')" \
...
pf_evm_require_equal "$signal2_values" "42 true" "bundleSignal after setAmount"
...
pf_evm_require_equal "$view2_values" "42 4 true" "bundleView after setAmount"
...
pf_evm_require_equal "$details2_values" "4 true" "detailsView after setAmount"
...
pf_evm_require_equal "$pairs2_values" "42 4" "amountSidePairs after setAmount"
...
pf_evm_require_storage "$addr" 3 42 "unauthorized setBundle holds amount"
...
pf_evm_require_storage "$addr" 3 42 "unauthorized setAmount holds amount"
