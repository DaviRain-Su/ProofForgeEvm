...
pf_evm_evm_init evm-anvil-allowlist
...
pf_evm_start_anvil "${PF_EVM_PORT:-18584}" "$root/build/evm/anvil-allowlist.log"
...
for slot in 3 4 5 6 7; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero enumerable state"
done
...
pf_evm_require_storage "$addr" 7 0 "unauthorized grant holds count"
...
pf_evm_require_storage "$addr" 3 0 "key zero occupies members[0]"
pf_evm_require_storage "$addr" 4 11 "members[1]"
pf_evm_require_storage "$addr" 5 22 "members[2]"
pf_evm_require_storage "$addr" 6 33 "members[3]"
...
pf_evm_require_storage "$addr" 7 4 "duplicate/full failures hold count"
pf_evm_require_storage "$addr" 6 33 "duplicate/full failures hold last value"
...
pf_evm_require_storage "$addr" 5 22 "last removal keeps stale backing slot"
pf_evm_require_storage "$addr" 7 2 "last removal decrements count"
pf_evm_require_equal "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 7 2 "absent revoke holds count"
...
pf_evm_require_storage "$addr" 7 0 "only removal reaches empty"
pf_evm_require_storage "$addr" 3 0 "only key zero remains stale as data"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_set_storage_word "$addr" "$payload_slot" 3
pf_evm_require_named_revert "$addr" "$sender" \
...
pf_evm_require_storage "$addr" 3 7 "forged-position failures hold backing"
pf_evm_require_storage "$addr" 7 1 "forged-position failures hold count"
pf_evm_require_storage "$addr" "$payload_slot" 3 "forged position is not repaired"
...
pf_evm_set_storage_word "$addr" "$payload_slot" 1
...
pf_evm_require_storage "$addr" 7 1 "backing mismatch holds count"
