...
pf_evm_evm_init evm-anvil-bitmap-claims
bin="$root/build/evm/EvmClaimBitmap.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18582}" "$root/build/evm/anvil-bitmap-claims.log"

bytecode="$(tr -d '\n\r ' < "$bin")"
addr="$(pf_evm_deploy_ctor_u64 "$bytecode" 0)"

for slot in 0 1 2; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero claimed word $slot"
done
...
pf_evm_require_storage "$addr" 0 9223372036854775808 "claim sets bit 63 (word-0 top bit)"
"$cast" send --rpc-url "$rpc" --private-key "$claimer_key" "$addr" 'claim(uint64)' 64 >/dev/null
pf_evm_require_storage "$addr" 1 1 "claim sets bit 64 as word-1 mask 1"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 2 2 "claim sets final bit 129 in partial word 2"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_named_revert "$addr" "$("$cast" wallet address --private-key "$claimer_key")" \
...
pf_evm_require_storage "$addr" 0 9223372036854775808 "replayed claim holds word 0"
...
pf_evm_require_named_revert "$addr" "$("$cast" wallet address --private-key "$private_key")" \
...
pf_evm_require_storage "$addr" 2 2 "OOB claims hold partial word 2 (no aliasing)"
pf_evm_require_storage "$addr" 0 9223372036854775808 "OOB claims hold word 0"
pf_evm_require_storage "$addr" 1 1 "OOB claims hold word 1"
...
pf_evm_require_storage "$addr" 0 9223372036854775812 "post-revert claim sets bit 2 in word 0"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
