...
pf_evm_evm_init evm-anvil-bitmap-flags
bin="$root/build/evm/EvmFeatureFlags.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18581}" "$root/build/evm/anvil-bitmap-flags.log"
...
owner_w1="${rest%% *}"
owner_w2="${rest#* }"
pf_evm_require_storage "$addr" 0 "$owner_w0" "constructor owner.w0"
pf_evm_require_storage "$addr" 1 "$owner_w1" "constructor owner.w1"
pf_evm_require_storage "$addr" 2 "$owner_w2" "constructor owner.w2"
pf_evm_require_storage "$addr" 3 0 "constructor zero flags word 0"
pf_evm_require_storage "$addr" 4 0 "constructor zero flags word 1"
...
pf_evm_require_storage "$addr" 3 0 "unauthorized enable holds word 0"
...
pf_evm_require_storage "$addr" 3 1 "enable writes flags[0] bit 0"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'enable(uint64)' 0 >/dev/null
pf_evm_require_storage "$addr" 3 1 "enable of a set bit is an idempotent same-word write"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'enable(uint64)' 63 >/dev/null
pf_evm_require_storage "$addr" 3 9223372036854775809 "enable sets the word-0 top bit"
pf_evm_require_storage "$addr" 4 0 "word-0 writes never touch word 1"
...
pf_evm_require_storage "$addr" 4 1 "enable writes flags[1] bit 64 as mask 1"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'isEnabled(uint64)(uint64)' 64)" 1 "boundary read"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'enable(uint64)' 127 >/dev/null
pf_evm_require_storage "$addr" 4 9223372036854775809 "enable sets the final in-range bit 127"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 3 9223372036854775809 "OOB enable holds word 0 (no aliasing)"
pf_evm_require_storage "$addr" 4 9223372036854775809 "OOB enable holds word 1"
...
pf_evm_require_storage "$addr" 4 "$word1_toggled" "toggle sets bit 100 in word 1"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'isEnabled(uint64)(uint64)' 100)" 1 "toggled read"
pf_evm_require_uint "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
  'toggle(uint64)(uint64)' 100)" 0 "second toggle clears and reports 0"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'toggle(uint64)' 100 >/dev/null
pf_evm_require_storage "$addr" 4 "$word1_base" "second toggle restores the exact word"
...
pf_evm_require_storage "$addr" 3 1 "disable clears bit 63 and keeps bit 0"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
