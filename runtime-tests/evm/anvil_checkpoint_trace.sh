...
pf_evm_evm_init evm-anvil-checkpoint-trace
...
pf_evm_start_anvil "${PF_EVM_PORT:-18687}" "$root/build/evm/anvil-checkpoint-trace.log"
...
for slot in 0 1 2 3 4 5 6; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero trace state"
done
...
pf_evm_require_storage "$addr" 0 5 "trace key 0"
pf_evm_require_storage "$addr" 1 10 "trace key 1"
pf_evm_require_storage "$addr" 2 20 "trace key 2"
pf_evm_require_storage "$addr" 3 50 "trace value 0"
pf_evm_require_storage "$addr" 4 100 "trace value 1"
pf_evm_require_storage "$addr" 5 200 "trace value 2"
pf_evm_require_storage "$addr" 6 3 "full trace count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'lowerValue(uint64)(uint64)' 6)" 100 "permissionless lower bound"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'latestValue()(uint64)')" 200 \
...
pf_evm_require_storage "$addr" 5 222 "typed trace latest overwrite"
pf_evm_require_storage "$addr" 6 3 "typed trace overwrite holds count"
...
pf_evm_set_storage_word "$addr" 6 4
...
pf_evm_require_storage "$addr" 6 4 "malformed count is not repaired"
pf_evm_require_storage "$addr" 5 222 "malformed count holds value"

pf_evm_set_storage_word "$addr" 6 2
pf_evm_set_storage_word "$addr" 1 5
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 1 5 "duplicate key is not repaired"
pf_evm_require_storage "$addr" 6 2 "duplicate key holds count"
