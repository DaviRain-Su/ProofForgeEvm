...
pf_evm_evm_init evm-anvil-config-map
...
pf_evm_start_anvil "${PF_EVM_PORT:-18588}" "$root/build/evm/anvil-config-map.log"
...
for slot in 3 4 5 6 7; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero table state"
done
...
pf_evm_require_storage "$addr" 7 0 "unauthorized write holds count"
...
for pair in '0 0' '11 110' '22 220' '33 330'; do
  key="${pair%% *}"
  value="${pair#* }"
  pf_evm_require_equal "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
    'write(uint64,uint64)(bool)' "$key" "$value")" true "write $key returns true"
  "$cast" send --rpc-url "$rpc" --private-key "$private_key" \
    "$addr" 'write(uint64,uint64)' "$key" "$value" >/dev/null
done
pf_evm_require_storage "$addr" 3 0 "key zero occupies keys[0]"
pf_evm_require_storage "$addr" 4 11 "keys[1]"
pf_evm_require_storage "$addr" 5 22 "keys[2]"
pf_evm_require_storage "$addr" 6 33 "keys[3]"
pf_evm_require_storage "$addr" 7 4 "full count"
...
pf_evm_require_storage "$addr" 7 4 "full update does not grow count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 7 4 "full growth failure holds count"
pf_evm_require_storage "$addr" "$(map_payload_slot 44 0)" 0 \
  "full growth writes no position"
pf_evm_require_storage "$addr" "$(map_payload_slot 44 1)" 0 \
...
pf_evm_require_storage "$addr" 4 33 "middle remove moves final key"
pf_evm_require_storage "$addr" 7 3 "middle remove decrements count"
pf_evm_require_storage "$addr" "$(map_payload_slot 11 0)" 0 \
  "middle remove clears removed position"
pf_evm_require_storage "$addr" "$(map_payload_slot 11 1)" 0 \
  "middle remove clears removed value"
pf_evm_require_storage "$addr" "$(map_payload_slot 33 0)" 2 \
  "middle remove repairs moved position"
pf_evm_require_storage "$addr" "$(map_payload_slot 33 1)" 330 \
...
pf_evm_require_storage "$addr" 5 22 "last remove keeps stale backing key"
pf_evm_require_storage "$addr" 7 2 "last remove decrements count"
pf_evm_require_storage "$addr" "$(map_payload_slot 22 0)" 0 \
  "last remove clears position"
pf_evm_require_storage "$addr" "$(map_payload_slot 22 1)" 0 \
...
pf_evm_require_storage "$addr" 7 2 "absent remove holds count"
...
pf_evm_require_storage "$addr" 7 0 "only remove reaches empty"
...
pf_evm_require_storage "$addr" 3 7 "forged failures hold backing"
pf_evm_require_storage "$addr" 7 1 "forged failures hold count"
pf_evm_require_storage "$addr" "$position_slot" 3 "forged position is not repaired"
pf_evm_require_storage "$addr" "$value_slot" 70 "forged failures hold value"
...
pf_evm_set_storage_word "$addr" 3 8
pf_evm_require_named_revert "$addr" "$sender" \
...
pf_evm_require_storage "$addr" 3 8 "backing mismatch is not repaired"
pf_evm_require_storage "$addr" "$value_slot" 70 "backing mismatch holds value"
...
pf_evm_require_storage "$addr" 7 5 "malformed mutations do not repair count"
