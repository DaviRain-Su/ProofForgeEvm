...
pf_evm_evm_init evm-anvil-id-registry
...
pf_evm_start_anvil "${PF_EVM_PORT:-18585}" "$root/build/evm/anvil-id-registry.log"
...
for slot in 0 1 2 3; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero registry state"
done
...
pf_evm_require_storage "$addr" 0 10 "ids[0]"
pf_evm_require_storage "$addr" 1 0 "key zero occupies ids[1]"
pf_evm_require_storage "$addr" 2 30 "ids[2]"
...
pf_evm_require_storage "$addr" 3 3 "duplicate/full failures hold count"
pf_evm_require_storage "$addr" 2 30 "duplicate/full failures hold last id"
...
pf_evm_require_storage "$addr" 1 30 "middle release moves final id"
pf_evm_require_storage "$addr" 3 2 "middle release decrements count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 1 30 "last release keeps stale backing slot"
pf_evm_require_storage "$addr" 3 1 "last release decrements count"
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 3 1 "absent release holds count"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'release(uint64)' 10 >/dev/null
pf_evm_require_storage "$addr" 3 0 "only release reaches empty"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'enroll(uint64)' 0 >/dev/null
pf_evm_require_storage "$addr" 0 0 "key zero reuses ids[0]"
pf_evm_require_storage "$addr" 3 1 "key zero re-enroll count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 0 0 "forged-position failures hold backing"
pf_evm_require_storage "$addr" 3 1 "forged-position failures hold count"
...
pf_evm_require_storage "$addr" 3 1 "backing mismatch holds count"
