...
pf_evm_evm_init evm-anvil-maybe
bin="$root/build/evm/Maybe.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18550}" "$root/build/evm/anvil-maybe.log"
...
require_leaves() {
  ...
  pf_evm_require_storage "$a" 0 "$tag" "$label tag"
  pf_evm_require_storage "$a" 1 "$payload" "$label payload"
}

require_leaves "$addr" 0 0 "ctor"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'isSome()(uint64)')" \
  0 "ctor isSome"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getValue()(uint64)')" \
  0 "ctor getValue"
