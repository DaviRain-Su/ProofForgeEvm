...
pf_evm_evm_init evm-anvil-pair
UINT64_MAX="18446744073709551615"
bin="$root/build/evm/Pair.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18548}" "$root/build/evm/anvil-pair.log"
...
require_pair() {
  ...
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getLeft()(uint64)')" \
    "$left" "$label getLeft"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getRight()(uint64)')" \
    "$right" "$label getRight"
  pf_evm_require_storage "$addr" 0 "$left" "$label storage left"
  pf_evm_require_storage "$addr" 1 "$right" "$label storage right"
}
...
pf_evm_require_uint "$simulated" 10 "creditLeft return"
