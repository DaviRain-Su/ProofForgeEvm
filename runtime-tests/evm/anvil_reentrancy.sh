...
pf_evm_evm_init evm-anvil-reentrancy
bin="$root/build/evm/GuardedPayout.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18566}" "$root/build/evm/anvil-reentrancy.log"
...
bytecode="$(tr -d '\n\r ' < "$bin")"
target="$(pf_evm_deploy_ctor_u64 "$bytecode" 0)"
recipient="0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC"
pf_evm_require_storage "$target" 0 1 "constructor initializes not-entered"

# Ordinary external call succeeds and restores the guard.
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$target" 'payout(address,uint256)' "$recipient" 0 >/dev/null
pf_evm_require_storage "$target" 0 1 "normal payout restores guard"

attacker_hex="$(tr -d '\n\r ' < "$attacker_out")"
attacker_receipt="$("$cast" send --json --rpc-url "$rpc" --private-key "$private_key" \
  --create "0x$attacker_hex")"
attacker="$(printf '%s' "$attacker_receipt" | pf_evm_contract_address)"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$attacker" 'callbacks()(uint256)')" \
  1 "hostile callback count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$attacker" 'observedStatus()(uint256)')" \
...
pf_evm_require_storage "$target" 0 1 "hostile outer call restores guard"
...
pf_evm_require_storage "$target" 0 1 "failed CALL rolls back entered sentinel"
