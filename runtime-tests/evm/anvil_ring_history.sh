...
pf_evm_evm_init evm-anvil-ring-history
...
pf_evm_start_anvil "${PF_EVM_PORT:-18583}" "$root/build/evm/anvil-ring-history.log"
...
pf_evm_require_storage "$addr" 0 7 "constructor seed tape[0]"
pf_evm_require_storage "$addr" 1 0 "constructor zero tape[1]"
pf_evm_require_storage "$addr" 2 0 "constructor zero tape[2]"
pf_evm_require_storage "$addr" 3 0 "constructor canonical head"
pf_evm_require_storage "$addr" 4 1 "constructor live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'liveOf()(uint64)')" 1 \
  "live getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'headOf()(uint64)')" 0 \
  "head getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 7 \
  "front view on seed"
...
pf_evm_require_storage "$addr" 1 8 "append writes tape[1]"
pf_evm_require_storage "$addr" 4 2 "append bumps live"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'append(uint64)' 9 >/dev/null
pf_evm_require_storage "$addr" 2 9 "append writes tape[2]"
pf_evm_require_storage "$addr" 4 3 "full tape live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 7 \
...
pf_evm_require_storage "$addr" 4 3 "full append holds live"
pf_evm_require_storage "$addr" 0 7 "full append holds tape[0]"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'advance()(uint64)')" 7 \
  "advance returns the front event"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'advance()' >/dev/null
pf_evm_require_storage "$addr" 3 1 "advance moves head"
pf_evm_require_storage "$addr" 4 2 "advance shrinks live"
pf_evm_require_storage "$addr" 0 7 "advanced slot stays stale and unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 8 \
...
pf_evm_require_storage "$addr" 0 10 "wrapped append overwrites stale tape[0]"
pf_evm_require_storage "$addr" 4 3 "wrapped append refills live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 8 \
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'advance()(uint64)')" 8 \
  "advance 1 returns the wrapped front"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'advance()' >/dev/null
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'advance()' >/dev/null
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'advance()' >/dev/null
pf_evm_require_storage "$addr" 3 0 "emptied advance canonicalizes head"
pf_evm_require_storage "$addr" 4 0 "emptied advance clears live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 4 0 "empty advance holds live"
...
pf_evm_require_storage "$addr" 0 42 "append after empty overwrites tape[0]"
pf_evm_require_storage "$addr" 1 8 "stale tape[1] stays unreachable"
pf_evm_require_storage "$addr" 4 1 "live after re-append"
...
pf_evm_require_storage "$addr" 3 0 "reset clears head"
pf_evm_require_storage "$addr" 4 0 "reset clears live"
pf_evm_require_storage "$addr" 0 42 "reset leaves stale tape[0] unreachable"
pf_evm_require_storage "$addr" 1 43 "reset leaves stale tape[1] unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 0 44 "post-reset append reuses tape[0]"
pf_evm_require_storage "$addr" 4 1 "post-reset live"

# Inject impossible metadata and prove no mutation silently repairs malformed persistent state.
pf_evm_set_storage_word "$addr" 3 3
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 3 3 "malformed mutations hold head"
pf_evm_require_storage "$addr" 4 1 "malformed mutations hold live"

pf_evm_set_storage_word "$addr" 4 4
pf_evm_require_named_revert "$addr" "$sender" \
  "$("$cast" calldata 'append(uint64)' 45)" 'malformed()' "malformed live append"
pf_evm_require_named_revert "$addr" "$sender" \
  "$("$cast" calldata 'advance()')" 'malformed()' "malformed live advance"
pf_evm_require_storage "$addr" 4 4 "malformed live mutations hold live"

# A masked non-canonical empty state (live 0 with head != 0) fails closed as well.
pf_evm_set_storage_word "$addr" 4 0
pf_evm_set_storage_word "$addr" 3 2
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'currentOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 3 2 "masked empty mutations hold head"
