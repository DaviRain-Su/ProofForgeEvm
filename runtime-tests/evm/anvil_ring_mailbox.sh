...
pf_evm_evm_init evm-anvil-ring-mailbox
...
pf_evm_start_anvil "${PF_EVM_PORT:-18582}" "$root/build/evm/anvil-ring-mailbox.log"
...
admin_w1="${rest%% *}"
admin_w2="${rest#* }"
pf_evm_require_storage "$addr" 0 "$admin_w0" "constructor admin.w0"
pf_evm_require_storage "$addr" 1 "$admin_w1" "constructor admin.w1"
pf_evm_require_storage "$addr" 2 "$admin_w2" "constructor admin.w2"
for slot in 3 4 5 6; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero pending slot"
done
pf_evm_require_storage "$addr" 7 0 "constructor canonical head"
pf_evm_require_storage "$addr" 8 0 "constructor empty live"
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'adminOf()(address)')" \
  "$sender" "admin getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'liveOf()(uint64)')" 0 \
  "empty live getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'headOf()(uint64)')" 0 \
  "canonical head getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 8 0 "unauthorized deliver holds live"
...
pf_evm_require_storage "$addr" 3 11 "deliver writes pending[0]"
pf_evm_require_storage "$addr" 8 1 "deliver bumps live"
...
pf_evm_require_storage "$addr" 6 44 "deliver writes pending[3]"
pf_evm_require_storage "$addr" 8 4 "full mailbox live"
for i in 0 1 2 3; do
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 8 4 "full deliver holds live"
pf_evm_require_storage "$addr" 3 11 "full deliver holds pending[0]"
...
pf_evm_require_storage "$addr" 7 1 "take advances head"
pf_evm_require_storage "$addr" 8 3 "take shrinks live"
pf_evm_require_storage "$addr" 3 11 "taken slot stays stale and unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 22 \
  "front view after take"

"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'deliver(uint64)' 55 >/dev/null
pf_evm_require_storage "$addr" 3 55 "wrapped delivery reuses pending[0]"
pf_evm_require_storage "$addr" 8 4 "wrapped delivery refills live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 22 \
...
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'take()' >/dev/null
pf_evm_require_storage "$addr" 7 0 "emptied take canonicalizes head"
pf_evm_require_storage "$addr" 8 0 "emptied take clears live"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 8 0 "empty take holds live"
...
pf_evm_require_storage "$addr" 7 0 "purge clears head"
pf_evm_require_storage "$addr" 8 0 "purge clears live"
pf_evm_require_storage "$addr" 3 66 "purge leaves stale pending[0] unreachable"
pf_evm_require_storage "$addr" 4 22 "purge leaves stale pending[1] unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 3 77 "post-purge delivery overwrites pending[0]"
pf_evm_require_storage "$addr" 4 22 "post-purge delivery leaves stale pending[1] unreachable"
pf_evm_require_storage "$addr" 8 1 "post-purge live"

# Inject impossible metadata and prove no mutation silently repairs malformed persistent state.
pf_evm_set_storage_word "$addr" 7 5
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 7 5 "malformed mutations hold head"
pf_evm_require_storage "$addr" 8 1 "malformed mutations hold live"

pf_evm_set_storage_word "$addr" 8 6
pf_evm_require_named_revert "$addr" "$sender" \
  "$("$cast" calldata 'deliver(uint64)' 88)" 'malformed()' "malformed live deliver"
pf_evm_require_named_revert "$addr" "$sender" \
  "$("$cast" calldata 'take()')" 'malformed()' "malformed live take"
pf_evm_require_storage "$addr" 8 6 "malformed live mutations hold live"

# A masked non-canonical empty state (live 0 with head != 0) fails closed as well.
pf_evm_set_storage_word "$addr" 8 0
pf_evm_set_storage_word "$addr" 7 2
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'frontOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 7 2 "masked empty mutations hold head"
