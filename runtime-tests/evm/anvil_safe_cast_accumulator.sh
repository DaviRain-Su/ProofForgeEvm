...
pf_evm_evm_init evm-anvil-safe-cast-accumulator
...
pf_evm_start_anvil "${PF_EVM_PORT:-18688}" \
...
pf_evm_require_storage "$addr" 0 0 "constructor total"
pf_evm_require_storage "$addr" 1 1 "constructor checkpoint"
pf_evm_require_storage "$addr" 2 2 "constructor batch"
pf_evm_require_storage "$addr" 3 3 "constructor mode"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'totalOf()(uint64)')" 0 \
  "initial total"
...
pf_evm_require_storage "$addr" 0 0 "zero add stores no nonzero value"
...
pf_evm_require_storage "$addr" 0 "$max64" "exact max persists"
...
pf_evm_require_storage "$addr" 0 "$max64" "sum overflow is atomic"
...
for case in \
  ...
  pf_evm_require_storage "$addr" 0 "$max64" "$label is atomic"
done

pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'totalOf()(uint64)')" \
...
pf_evm_require_storage "$addr" 1 1 "zero checkpoint is atomic"

pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'setCheckpoint(uint256)(uint32)' "$max32")" "$max32" "exact UInt32 max"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setCheckpoint(uint256)' "$max32" >/dev/null
pf_evm_require_storage "$addr" 1 "$max32" "exact UInt32 max persists"
...
for case in \
  ...
  pf_evm_require_storage "$addr" 1 "$max32" "$label is atomic"
done
pf_evm_require_storage "$addr" 0 "$max64" "checkpoint path preserves UInt64 total"
...
pf_evm_require_storage "$addr" 2 2 "zero batch is atomic"

pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'setBatch(uint256)(uint16)' "$max16")" "$max16" "exact UInt16 max"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setBatch(uint256)' "$max16" >/dev/null
pf_evm_require_storage "$addr" 2 "$max16" "exact UInt16 max persists"
...
for case in \
  ...
  pf_evm_require_storage "$addr" 2 "$max16" "$label is atomic"
done
pf_evm_require_storage "$addr" 0 "$max64" "batch path preserves UInt64 total"
pf_evm_require_storage "$addr" 1 "$max32" "batch path preserves UInt32 checkpoint"
...
pf_evm_require_storage "$addr" 3 3 "zero mode is atomic"

pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
  'setMode(uint256)(uint8)' "$max8")" "$max8" "exact UInt8 max"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setMode(uint256)' "$max8" >/dev/null
pf_evm_require_storage "$addr" 3 "$max8" "exact UInt8 max persists"
...
for case in \
  ...
  pf_evm_require_storage "$addr" 3 "$max8" "$label is atomic"
done
pf_evm_require_storage "$addr" 0 "$max64" "mode path preserves UInt64 total"
pf_evm_require_storage "$addr" 1 "$max32" "mode path preserves UInt32 checkpoint"
pf_evm_require_storage "$addr" 2 "$max16" "mode path preserves UInt16 batch"
