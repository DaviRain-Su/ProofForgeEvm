...
pf_evm_evm_init evm-anvil-safe-cast-config
...
pf_evm_start_anvil "${PF_EVM_PORT:-18689}" "$root/build/evm/anvil-safe-cast-config.log"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'limitOf()(uint64)')" 7 \
  "constructor limit"
pf_evm_require_storage "$addr" 3 7 "constructor limit slot"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'windowOf()(uint32)')" 3 \
  "constructor window"
pf_evm_require_storage "$addr" 4 3 "constructor window slot"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'thresholdOf()(uint16)')" 5 \
  "constructor threshold"
pf_evm_require_storage "$addr" 5 5 "constructor threshold slot"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'levelOf()(uint8)')" 6 \
  "constructor level"
pf_evm_require_storage "$addr" 6 6 "constructor level slot"
...
pf_evm_require_storage "$addr" 3 7 "zero rejection is atomic"

# Exact UInt64 max is accepted and replaces the prior limit.
pf_evm_require_uint "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
  'setLimit(uint128)(uint64)' "$max64")" "$max64" "exact UInt64 max"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setLimit(uint128)' "$max64" >/dev/null
pf_evm_require_storage "$addr" 3 "$max64" "exact max persists"
...
for case in "$just_over|just-overflow" "$high_limb|high-limb overflow"; do
  ...
  pf_evm_require_storage "$addr" 3 "$max64" "$label is atomic"
done
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'limitOf()(uint64)')" \
...
pf_evm_require_storage "$addr" 4 3 "zero window is atomic"

pf_evm_require_uint "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
  'setWindow(uint128)(uint32)' "$max32")" "$max32" "exact UInt32 max window"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setWindow(uint128)' "$max32" >/dev/null
pf_evm_require_storage "$addr" 4 "$max32" "exact UInt32 max window persists"
...
pf_evm_require_storage "$addr" 3 "$max64" "window path preserves UInt64 limit"
...
pf_evm_require_storage "$addr" 5 5 "zero threshold is atomic"

pf_evm_require_uint "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
  'setThreshold(uint128)(uint16)' "$max16")" "$max16" "exact UInt16 max threshold"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setThreshold(uint128)' "$max16" >/dev/null
pf_evm_require_storage "$addr" 5 "$max16" "exact UInt16 max threshold persists"
...
pf_evm_require_storage "$addr" 3 "$max64" "threshold path preserves UInt64 limit"
pf_evm_require_storage "$addr" 4 "$max32" "threshold path preserves UInt32 window"
...
pf_evm_require_storage "$addr" 6 6 "zero level is atomic"

pf_evm_require_uint "$("$cast" call --from "$sender" --rpc-url "$rpc" "$addr" \
  'setLevel(uint128)(uint64)' "$max8")" "$max8" "exact UInt8 max level"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setLevel(uint128)' "$max8" >/dev/null
pf_evm_require_storage "$addr" 6 "$max8" "exact UInt8 max level persists"
...
pf_evm_require_storage "$addr" 3 "$max64" "level path preserves UInt64 limit"
pf_evm_require_storage "$addr" 4 "$max32" "level path preserves UInt32 window"
pf_evm_require_storage "$addr" 5 "$max16" "level path preserves UInt16 threshold"
