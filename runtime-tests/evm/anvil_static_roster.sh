...
for slot in 3 4 5 6 7 8 9; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero seat/closed slot"
done
...
pf_evm_require_storage "$addr" 5 42 "seat[1].points targeted mutation"
pf_evm_require_storage "$addr" 6 3 "seat[1].tier targeted mutation"
for slot in 3 4 7 8 9; do
  pf_evm_require_storage "$addr" "$slot" 0 "neighboring seat/closed slot holds"
done
...
pf_evm_require_storage "$addr" 5 42 "OOB update holds selected seat"
pf_evm_require_storage "$addr" 7 0 "OOB update cannot spill into next slot"
...
for slot in 10 11 12 13 14 15; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor writer slot vacant"
done
...
for slot in 10 11 12; do
  pf_evm_require_storage "$addr" "$slot" 0 "unauthorized grant holds writer slot0"
done
...
pf_evm_require_storage "$addr" 12 "$ow2" "grant writer slot0 w2"
for slot in 13 14 15; do
  pf_evm_require_storage "$addr" "$slot" 0 "grant leaves writer slot1 vacant"
done
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 15 "$pw2" "grant writer slot1 w2"
...
pf_evm_require_storage "$addr" 13 "$pw0" "unauthorized revoke holds writer slot1"

"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'close()' >/dev/null
pf_evm_require_storage "$addr" 9 1 "closed flag targeted mutation"
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'closedOf()(bool)')" \
  true "closed getter"
...
pf_evm_require_storage "$addr" 5 42 "closed update holds points"
pf_evm_require_storage "$addr" 6 3 "closed update holds tier"
...
pf_evm_require_storage "$addr" 13 "$pw0" "closed revoke holds writer slot1"
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" \
