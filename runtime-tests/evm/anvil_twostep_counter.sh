...
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'pendingOwner()(address)')" \
  "0x0000000000000000000000000000000000000000" "no initial pendingOwner"
...
pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'pendingOwner()(address)')" \
  "0x0000000000000000000000000000000000000000" "pendingOwner cleared after accept"
...
if [[ -z "$solc_bin" ]]; then
  echo "evm-anvil-twostep-counter: ok (Access gates + CREATE OwnershipTransferred + zero-owner CREATE, solc skip)"
  exit 0
fi
yul="$root/build/evm/TwoStepCounter.yul"
...
if not hexes:
    raise SystemExit('FAIL: solc produced no TwoStepCounter constructor-guard mutation bytecode')
print(hexes[-1])
")"
...
log_mut_dir="$root/build/evm/twostep-ctor-log-mut"
rm -rf "$log_mut_dir"
mkdir -p "$log_mut_dir"
pf_evm_strip_ctor_ownership_log "$yul" TwoStepCounter "$log_mut_dir/TwoStepCounter.yul"
log_mut_code="$("$solc_bin" --strict-assembly --optimize --evm-version cancun --bin \
  "$log_mut_dir/TwoStepCounter.yul" | "$python" -I -S -c "
import sys
lines=[ln.strip() for ln in sys.stdin.read().splitlines() if ln.strip()]
hexes=[ln for ln in lines if len(ln)>100 and all(c in '0123456789abcdefABCDEF' for c in ln)]
if not hexes:
    raise SystemExit('FAIL: solc produced no TwoStepCounter constructor-log mutation bytecode')
print(hexes[-1])
")"
...
echo "evm-anvil-twostep-counter: ok (Access gates + CREATE OwnershipTransferred + zero-owner CREATE + constructor mutations + ownership transfer/renounce LOG3 + Paused/Unpaused LOG1)"
