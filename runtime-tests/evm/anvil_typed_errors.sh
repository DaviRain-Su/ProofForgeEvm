...
pf_evm_evm_init evm-anvil-typed-errors
...
pf_evm_start_anvil "${PF_EVM_PORT:-18688}" "$root/build/evm/anvil-typed-errors.log"

bytecode="$(tr -d '\n\r ' < "$bin")"
sender="$("$cast" wallet address --private-key "$private_key")"
addr="$(pf_evm_deploy_ctor_u64 "$bytecode" 3)"

pf_evm_require_storage "$addr" 0 3 "constructor value"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'valueOf()(uint64)')" 3 \
  "initial value"
...
for case in '5 1' '3 7' '5 8' '0 7'; do
  ...
  pf_evm_require_storage "$addr" 0 3 "typed error is atomic"
done
...
pf_evm_require_storage "$addr" 0 5 "successful update persists"

pf_evm_require_word_revert "$addr" "$sender" \
  "$("$cast" calldata 'update(uint64,uint64)' 9 2)" 'denied(uint64)' \
  "post-success error" 2
pf_evm_require_storage "$addr" 0 5 "post-success revert remains atomic"
