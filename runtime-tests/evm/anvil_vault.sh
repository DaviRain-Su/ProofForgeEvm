...
pf_evm_evm_init evm-anvil-vault
bin="$root/build/evm/Vault.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18554}" "$root/build/evm/anvil-vault.log"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getU64(uint64)(uint64)' 7)" \
  0 "absent map u64"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'setU64(uint64,uint64)' 7 9 >/dev/null
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getU64(uint64)(uint64)' 7)" \
  9 "map u64 after set"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'getU64(uint64)(uint64)' 8)" \
  0 "other key stays absent"

sender="$("$cast" wallet address --private-key "$private_key")"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'shareOf(address)(uint256)' "$sender")" \
  0 "absent share"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
...
weth="$(printf '%s' "$weth_receipt" | pf_evm_contract_address)"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'held(address)(uint256)' "$weth")" \
  40 "held WETH after wrap"
after_wrap_eth="$("$cast" balance --rpc-url "$rpc" "$addr")"
pf_evm_require_uint "$after_wrap_eth" "$(pf_evm_to_dec "$before_eth")" \
...
pf_evm_require_uint "$after_unwrap_eth" \
  "$("$python" -I -S -c "print(int('$(pf_evm_to_dec "$before_eth")') + 15)")" \
...
router="$(printf '%s' "$router_receipt" | pf_evm_contract_address)"

token_b_receipt="$("$cast" send --json --rpc-url "$rpc" --private-key "$private_key" --create "0x$mock_hex")"
token_b="$(printf '%s' "$token_b_receipt" | pf_evm_contract_address)"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'held(address)(uint256)' "$token_b")" \
  30 "token B after swap2"

token_c_receipt="$("$cast" send --json --rpc-url "$rpc" --private-key "$private_key" --create "0x$mock_hex")"
token_c="$(printf '%s' "$token_c_receipt" | pf_evm_contract_address)"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'held(address)(uint256)' "$token_c")" \
  20 "token C after swap3"

internal_bin="$root/build/evm/Token.bin"
pf_evm_ensure_bin "$internal_bin"
internal_hex="$(tr -d '\n\r ' < "$internal_bin")"
internal="$(pf_evm_deploy_ctor_address "$internal_hex" "$sender")"
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$internal" \
