...
pf_evm_evm_init evm-anvil-vec-log
bin="$root/build/evm/EvmVecLog.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18572}" "$root/build/evm/anvil-vec-log.log"
...
for slot in 3 4 5 6 7; do
  pf_evm_require_storage "$addr" "$slot" 0 "constructor zero entry/count slot"
done
...
pf_evm_require_storage "$addr" 7 0 "unauthorized record holds count"
...
pf_evm_require_storage "$addr" 3 11 "record writes entries[0]"
pf_evm_require_storage "$addr" 7 1 "record bumps count"
...
pf_evm_require_storage "$addr" 4 22 "record writes entries[1]"
pf_evm_require_storage "$addr" 5 33 "record writes entries[2]"
pf_evm_require_storage "$addr" 6 44 "record writes entries[3]"
pf_evm_require_storage "$addr" 7 4 "full log count"
for i in 0 1 2 3; do
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 7 4 "full record holds count"
pf_evm_require_storage "$addr" 3 11 "full record holds entries[0]"

# Amend rewrites exactly one active slot; neighbors hold.
"$cast" send --rpc-url "$rpc" --private-key "$private_key" \
  "$addr" 'amend(uint64,uint64)' 1 99 >/dev/null
pf_evm_require_storage "$addr" 4 99 "amend writes entries[1]"
pf_evm_require_storage "$addr" 3 11 "amend holds entries[0]"
pf_evm_require_storage "$addr" 5 33 "amend holds entries[2]"
pf_evm_require_storage "$addr" 7 4 "amend holds count"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 4 99 "OOB amend holds entries[1]"
...
pf_evm_require_storage "$addr" 7 4 "unauthorized wipe holds count"
...
pf_evm_require_storage "$addr" 7 0 "wipe clears count"
pf_evm_require_storage "$addr" 3 11 "wipe leaves stale entries[0] unreachable"
pf_evm_require_storage "$addr" 4 99 "wipe leaves stale entries[1] unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" \
...
pf_evm_require_storage "$addr" 3 55 "post-wipe record overwrites entries[0]"
pf_evm_require_storage "$addr" 4 99 "post-wipe record leaves stale entries[1] unreachable"
pf_evm_require_storage "$addr" 7 1 "post-wipe count"
...
pf_evm_require_storage "$addr" 7 5 "malformed mutations hold count"
pf_evm_require_storage "$addr" 3 55 "malformed mutations hold entries[0]"
