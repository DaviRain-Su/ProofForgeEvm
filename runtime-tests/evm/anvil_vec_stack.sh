...
pf_evm_evm_init evm-anvil-vec-stack
bin="$root/build/evm/EvmVecStack.bin"
pf_evm_ensure_bin "$bin"
pf_evm_start_anvil "${PF_EVM_PORT:-18573}" "$root/build/evm/anvil-vec-stack.log"

bytecode="$(tr -d '\n\r ' < "$bin")"
sender="$("$cast" wallet address --private-key "$private_key")"
addr="$(pf_evm_deploy_ctor_u64 "$bytecode" 7)"

pf_evm_require_storage "$addr" 0 7 "constructor seed items[0]"
pf_evm_require_storage "$addr" 1 0 "constructor zero items[1]"
pf_evm_require_storage "$addr" 2 0 "constructor zero items[2]"
pf_evm_require_storage "$addr" 3 1 "constructor depth"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'depthOf()(uint64)')" 1 \
  "depth getter"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'topOf()(uint64)')" 7 \
  "top view on seed"

# Permissionless pushes fill the active prefix.
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'push(uint64)' 8 >/dev/null
pf_evm_require_storage "$addr" 1 8 "push writes items[1]"
pf_evm_require_storage "$addr" 3 2 "push bumps depth"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'push(uint64)' 9 >/dev/null
pf_evm_require_storage "$addr" 2 9 "push writes items[2]"
pf_evm_require_storage "$addr" 3 3 "full stack depth"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'topOf()(uint64)')" 9 \
...
pf_evm_require_storage "$addr" 3 3 "full push holds depth"
pf_evm_require_storage "$addr" 0 7 "full push holds items[0]"

# eth_call observes pop's return value without mutating; the real pop then shrinks depth and
# leaves the popped slot stale.
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'pop()(uint64)')" 9 \
  "pop returns the top value"
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'pop()' >/dev/null
pf_evm_require_storage "$addr" 3 2 "pop shrinks depth"
pf_evm_require_storage "$addr" 2 9 "popped slot stays stale and unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'topOf()(uint64)')" 8 \
  "top view after pop"

"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'pop()' >/dev/null
"$cast" send --rpc-url "$rpc" --private-key "$private_key" "$addr" 'pop()' >/dev/null
pf_evm_require_storage "$addr" 3 0 "empty stack depth"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'topOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 3 0 "empty pop holds depth"
...
pf_evm_require_storage "$addr" 0 42 "push after empty overwrites items[0]"
pf_evm_require_storage "$addr" 1 8 "stale items[1] stays unreachable"
pf_evm_require_storage "$addr" 3 1 "depth after re-push"
...
pf_evm_require_storage "$addr" 3 0 "clearAll clears depth"
pf_evm_require_storage "$addr" 0 42 "clearAll leaves stale items[0] unreachable"
pf_evm_require_storage "$addr" 1 43 "clearAll leaves stale items[1] unreachable"
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'topOf()(uint64)')" 0 \
...
pf_evm_require_storage "$addr" 3 4 "malformed mutations hold depth"
pf_evm_require_storage "$addr" 0 42 "malformed mutations hold items[0]"
