...
pf_evm_evm_init evm-anvil-yulc-capped
pf_evm_yulc_or_skip evm-anvil-yulc-capped

out="$root/build/evm-yulc-anvil-capped"
pf_evm_dual_build_program Capped "$out"
pf_evm_dual_bytecode_note evm-anvil-yulc-capped

pf_evm_start_anvil "${PF_EVM_PORT:-18591}" "$root/build/evm/anvil-yulc-capped.log"
...
run_capped_suite() {
  ...
  pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'ownerOf()(address)' | tr '[:upper:]' '[:lower:]')" \
    "${sender,,}" "$label: ownerOf"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'pausedOf()(uint8)')" \
    0 "$label: initial paused"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'capOf()(uint256)')" \
    100 "$label: cap"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'totalSupply()(uint256)')" \
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$solc_addr" 'totalSupply()(uint256)')" \
