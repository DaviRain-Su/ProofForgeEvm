...
pf_evm_evm_init evm-anvil-yulc-const
pf_evm_yulc_or_skip evm-anvil-yulc-const

out="$root/build/evm-yulc-anvil-const"
pf_evm_dual_build_program Const "$out"
pf_evm_dual_bytecode_note evm-anvil-yulc-const

pf_evm_start_anvil "${PF_EVM_PORT:-18592}" "$root/build/evm/anvil-yulc-const.log"
...
run_const_suite() {
  ...
  pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'whoOf()(address)' | tr '[:upper:]' '[:lower:]')" \
    "${sender,,}" "$label: immutable who"
  pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'peerOf()(address)' | tr '[:upper:]' '[:lower:]')" \
