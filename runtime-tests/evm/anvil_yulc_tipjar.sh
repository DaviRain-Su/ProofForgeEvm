...
pf_evm_evm_init evm-anvil-yulc-tipjar
pf_evm_yulc_or_skip evm-anvil-yulc-tipjar

out="$root/build/evm-yulc-anvil-tipjar"
pf_evm_dual_build_program TipJar "$out"
pf_evm_dual_bytecode_note evm-anvil-yulc-tipjar

pf_evm_start_anvil "${PF_EVM_PORT:-18596}" "$root/build/evm/anvil-yulc-tipjar.log"
...
run_tipjar_suite() {
  ...
  pf_evm_require_equal "$after" "$want" "$label: payout credit"
  pf_evm_require_equal "$(pf_evm_to_dec "$("$cast" balance --rpc-url "$rpc" "$addr")")" \
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$solc_addr" 'selfBal()(uint256)')" \
