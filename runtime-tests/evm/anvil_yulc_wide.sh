...
pf_evm_evm_init evm-anvil-yulc-wide
pf_evm_yulc_or_skip evm-anvil-yulc-wide

out="$root/build/evm-yulc-anvil-wide"
pf_evm_dual_build_program Wide "$out"
pf_evm_dual_bytecode_note evm-anvil-yulc-wide

pf_evm_start_anvil "${PF_EVM_PORT:-18595}" "$root/build/evm/anvil-yulc-wide.log"
...
run_wide_suite() {
  ...
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'echo(uint256)(uint256)' 7)" \
    7 "$label: echo uint256"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'add(uint256,uint256)(uint256)' 1 2)" \
    3 "$label: 1+2"

  wide="$("$python" -I -S -c "print((1<<64)+1)")"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'add(uint256,uint256)(uint256)' "$wide" 2)" \
    "$("$python" -I -S -c "print((1<<64)+3)")" "$label: wide add"
  pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$addr" 'sub(uint256,uint256)(uint256)' "$wide" 1)" \
    "$("$python" -I -S -c "print(1<<64)")" "$label: wide sub"

  cmp_a="$("$python" -I -S -c "print((1<<192)+(7<<64)+3)")"
  cmp_b="$("$python" -I -S -c "print((1<<192)+(8<<64)+1)")"
  pf_evm_require_equal "$("$cast" call --rpc-url "$rpc" "$addr" 'lt256(uint256,uint256)(bool)' "$cmp_a" "$cmp_b")" \
...
pf_evm_require_uint "$("$cast" call --rpc-url "$rpc" "$solc_addr" 'echo(uint256)(uint256)' 99)" \
