...
- `ProofForge/Evm/Sdk/**` must not import Emit / Assemble / Registry.
"""
...
DIRECT_EMIT_IMPORT = re.compile(
    r"^\s*import\s+ProofForge\.Evm(?:\.[A-Za-z0-9_]+)*\.Emit\s*(?:--.*)?$",
    re.MULTILINE,
)
...
SDK_FORBIDDEN_IMPORT = re.compile(
    r"^\s*import\s+ProofForge\.(?P<target>Evm)(?:\.[A-Za-z0-9_]+)*\."
...
def main() -> int:
    ...
    for path in iter_sdk_files():
        ...
        for match in SDK_FORBIDDEN_IMPORT.finditer(text):
            # Only same-target leakage is in scope for P0 (Evm Sdk → Evm.Emit, etc.).
