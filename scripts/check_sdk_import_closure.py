#!/usr/bin/env python3
"""Assert ProofForge.Evm.Sdk transitive imports never reach Emit/Assemble/Registry.
...
def module_to_path(mod: str) -> Path | None:
    ...
    # ProofForge.Evm.Sdk → ProofForge/Evm/Sdk.lean
...
def is_forbidden(mod: str) -> bool:
    # Same-target backend surfaces only (Evm).
    for target in ("Evm",):
        ...
        if head in ("Emit", "Assemble", "Registry"):
            return True
        # Nested: ProofForge.Evm.AccountStorage.Emit (pattern; Evm has no such dir today)
