...
const BRANCHES = [
  {
    ...
    out: { zh: "solc .bin / .yul", en: "solc .bin / .yul" },
  },
  {
    id: "yulc",
        ...
        <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-4">
          {BRANCHES.map((b) => (
            <div key={b.id} className="flex min-w-0 flex-col items-center">
              <Node n={b.id === "solc" ? "04a" : "04b"} title={copy(lang, b.ir)} body={copy(lang, b.irBody)} />
              <Stem />
              <Node n={b.id === "solc" ? "05a" : "05b"} title={copy(lang, b.out)} body={b.id.toUpperCase()} mute />
            </div>
          ))}
        </div>
      </div>
